  ...
  <body>
    <div class="x-nav">
      <span class="layui-breadcrumb">
        <a href="">首页</a>
        <a href="">出库管理</a>
        <a href="${ctx }/pmsPurchases/page/list">
          <cite>原料出库申请</cite></a>
      </span>
    ...
    <div class="x-body">
      ...
      <div class="demoTable">
        ...
        <div class="layui-inline">
          <input class="layui-input" name="rawMatrialId" id="rawMatrialId" autocomplete="off">
        </div>

        合同ID：
        <div class="layui-inline">
          <input class="layui-input" name="contactId" id="contactId" autocomplete="off">
        </div>
        ...
        <button id="do_add" class="layui-btn layui-btn-normal" data-type="reload">添加</button>
          ...
          <button id="do_reload" class="layui-btn layui-btn-normal" data-type="reload" onclick="window.location.reload()" style="position:relative;left: 420px;">刷新</button>
      ...
      <%--<table id="list-permission" class="layui-hide"  lay-filter="permissionTableFilter"></table>--%>
        <table class="layui-table" lay-even="true"
               ...
               lay-data="{
                            url: '${ctx}/materialDeliveryOrders/list' //③数据接口 ,直接去controller
                            ,id: 'materialDeliveryOrderTableId'
                            ,height: 500
            ...
            <tr>
                ...
                <th lay-data="{field: 'orderId', width:150}">对应订单编号</th>
                <th lay-data="{field: 'materialId',sort: true, width:100}">物料ID</th> <%--,  fixed: 'left'--%>
                <th lay-data="{field: 'productName', width:200}">物料名</th>
                <th lay-data="{field: 'materialNum', width:100}">申请数量</th>
                <th lay-data="{field: 'status', width:100}">申请状态</th>
        ...
        <script type="text/html" id="barDemo">
            ...
            <a class="layui-btn layui-btn-xs" lay-event="edit" data-d="确认出库" onmouseover="show_sth(this)">确认出库</a>
            <a class="layui-btn layui-btn-danger layui-btn-xs" lay-event="del">删除</a>
        </script>
        <script>

            function show_sth(t) {
                var row = $(t).attr("data-d");  //G：获取显示内容
                //小tips
                layer.tips(row,t,{
                    tips:[1,'#3595CC'],
                    time:1000
            ...
            layui.use('table', function(){
                ...
                table.on('toolbar(permissionTableFilter)', function(obj) {
                  ...
                  switch (obj.event) {
                    case 'add':
                       <%--x_admin_show('添加','${ctx}/pmsPurchases/page/add');--%>
                        break;
                    case 'update':
                       if (data.length === 0) {
                            layer.msg('请选择一行');
                            ...
                            x_admin_show('状态修改','${ctx}/pmsPurchases/page/update?id='+checkStatus.data[0].id);
                        }
                        break;
                    case 'delete':
                      if (data.length === 0) {
                ...
                table.on('tool(permissionTableFilter)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
                   ...
                   /*if(layEvent === 'detail'){
                    ...
                    if(layEvent === 'del'){// 删除行
                      ...
                      x_admin_show('状态修改','${ctx}/pmsPurchases/page/update?id='+data.id);
                      //layer.msg(data.id);
                    }
                });
                  ...
                  $('#do_search').on('click', function () {
                    // 搜索条件
                    var rawMatrialId = $('#rawMatrialId').val();
                    var contactId = $('#contactId').val();

                    table.reload('materialDeliveryOrderTableId', {
                      method: 'get'
                      , where: {
                        'rawMatrialId': rawMatrialId,
                        'contactId': contactId
                      }
                      , page: {
                        curr: 1
