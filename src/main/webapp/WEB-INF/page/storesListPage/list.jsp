    ...
    <div class="x-body">
      ...
      <%--<table id="list-permission" class="layui-hide"  lay-filter="permissionTableFilter"></table>--%>
        <table class="layui-table" lay-even="true"
               ...
               lay-data="{
                            url: '${ctx}/storesLists/list' //③数据接口 ,直接去controller
                            ,id: 'storesListTableId'
                            ,height: 500
        ...
        >
            ...
            <tr>
                ...
                <%--<th lay-data="{field: 'inventoryId', width:100, sort: true, fixed: 'left'}">ID</th>--%>
                <th lay-data="{field: 'materialId', sort: true, width:100}">物料ID</th>
                <th lay-data="{field: 'productName', width:100}">物料名</th>   <%--基础表中--%>
                <th lay-data="{field: 'bigTypeId', width:100}">所属大类</th>
                <th lay-data="{field: 'smallTypeId', width:100}">所属小类</th>
                <th lay-data="{field: 'specificationType', width:100}">规格</th>    <%--基础表中--%>
                <th lay-data="{field: 'inventoryNum', width:100}">库存数量</th>
                <th lay-data="{field: 'usableInventoryNum', width:100}">可用库存数量</th>
        ...
        <script>
            layui.use('table', function(){
                ...
                table.on('tool(permissionTableFilter)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
                   ...
                   /*if(layEvent === 'detail'){
                    ...
                    if(layEvent === 'del'){// 删除行
                      ...
                      x_admin_show('状态修改','${ctx}/storesLists/page/update?id='+data.id);
                      //layer.msg(data.id);
                    }
                });
                  ...
                  $('#do_search').on('click', function () {
                    ...
                    table.reload('storesListTableId', {
                      method: 'get'
                      , where: {
                ...
                $('#do_add').on('click', function () {s
                  x_admin_show('添加','${ctx}/storeLists/page/add');
                });
