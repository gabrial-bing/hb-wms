  ...
  <body>
    <div class="x-nav">
      <span class="layui-breadcrumb">
        <a href="">首页</a>
        <a href="">出库管理</a>
        <a href="${ctx }/commodityStockOuts/page/list">
          <cite>成品出库记录</cite></a>
      </span>
    ...
    <div class="x-body">
      ...
      <div class="demoTable">
        ...
        出库人员：
        <div class="layui-inline">
          <input class="layui-input" name="outStorageName" id="outStorageName" autocomplete="off">
        </div>
      ...
      <%--<table id="list-permission" class="layui-hide"  lay-filter="permissionTableFilter"></table>--%>
        <table class="layui-table" lay-even="true"
               ...
               lay-data="{
                            url: '${ctx}/commodityStockOuts/list' //③数据接口 ,直接去controller
                            ,id: 'commodityStockOutTableId'
                            ,height: 500
        ...
        >
            ...
            <tr>
               ...
               <%-- <th lay-data="{field: 'stockOutId', width:100, sort: true, fixed: 'left'}">ID</th>--%>
                <th lay-data="{field: 'orderId',sort: true, width:100}">订单号</th>
                <th lay-data="{field: 'clientName', width:100}">客户名称</th>
                <th lay-data="{field: 'commodityId', width:100}">成品编号</th>
                <th lay-data="{field: 'inlandOrOut', width:100}">国内(外)</th>
                <th lay-data="{field: 'storageLocation', width:100}">仓储位置</th>
                <th lay-data="{field: 'outStorageNum', width:100}">出库数量</th>
                <th lay-data="{field: 'outStorageName', width:100}">出库申请人</th>
                <th lay-data="{field: 'outStorageDate', width:100}">出库时间</th>
        ...
        <script>
            layui.use('table', function(){
                  ...
                  switch (obj.event) {
                    ...
                    case 'update':
                        if (data.length === 0) {
                            ...
                            x_admin_show('状态修改','${ctx}/commodityStockOuts/page/update?id='+checkStatus.data[0].id);
                        }
                        break;
                    case 'delete':
                      if (data.length === 0) {
                          ...
                          for (var i = 0; i < checkStatus.data.length; i++) {
                            $.ajax({
                              url: "${ctx}/commodityStockOuts/delete",
                              type: "post",
                              dataType: "json",
                ...
                table.on('tool(permissionTableFilter)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
                   ...
                   /*if(layEvent === 'detail'){
                    ...
                    if(layEvent === 'del'){// 删除行
                      ...
                      x_admin_show('状态修改','${ctx}/commodityStockOuts/page/update?id='+data.id);
                      //layer.msg(data.id);
                    }
                });
                  ...
                  $('#do_search').on('click', function () {
                    ...
                    var outStorageName = $('#outStorageName').val();

                    table.reload('commodityStockOutTableId', {
                      method: 'get'
                      , where: {
                        'orderId': orderId,
                ...
                $('#do_add').on('click', function () {
                  x_admin_show('添加','${ctx}/commodityStockOuts/page/add');
                });
